...
- A fresh run is **Manual host**: arrived parties wait for drag-and-drop or party-then-table selection, and nothing self-assigns. Attach a WebMCP agent and the header switches to **Agent: name**; the agent plans the whole night (a tentative table for every reservation, so scarce tables are protected for later requests), is asked to re-plan after every event and every 10 minutes, and each plan executes by itself when the party arrives, falling back to the agent's next-ranked table if the first is no longer legal. Free tables show who is planned there next. `detach_agent` returns to Manual with tentative plans cleared and seatings preserved.
